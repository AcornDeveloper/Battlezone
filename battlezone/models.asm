...
;
; render byte 0 plot point, 1 move/draw, 2-6 vertice, 7 exit
...
MACRO vertice_explosion x, y, z, m
 EQUW - x * bz_scale_factor * m
 EQUW - y * bz_scale_factor * m
 EQUW   z * bz_scale_factor * m
ENDMACRO
...
.exhaust_object_vertices_00
 FOR degree, 0, 359, 360 / 8
  vertice_exhaust COS(RAD(degree)) * 64, &40, SIN(RAD(degree)) * 64, 0.40
 NEXT
...
.exhaust_object_vertices_01
 FOR degree, 0, 359 , 360 / 8
  vertice_exhaust COS(RAD(degree)) * 64, &40, SIN(RAD(degree)) * 64, 0.80
 NEXT
...
.exhaust_object_vertices_02
 FOR degree, 0, 359, 360 / 8
  vertice_exhaust COS(RAD(degree)) * 64, &40, SIN(RAD(degree)) * 64, 1.20
 NEXT
...
.exhaust_object_vertices_03
 FOR degree, 0, 359, 360 / 8
  vertice_exhaust COS(RAD(degree)) * 64, &40, SIN(RAD(degree)) * 64, 1.60
 NEXT
